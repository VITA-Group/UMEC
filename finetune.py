...
if __name__ == "__main__":
    ...
    if args.save_model == '':
        args.save_model = os.path.join(args.save_result_dir, 'best.pt')
    ...
    if args.mask_file:
        ...
        # dlrm_weights_mask = pickle.load(args.mask_file)
    ...
    with torch.autograd.profiler.profile(args.enable_profiling, use_gpu) as prof:
        while k < args.nepochs:
            ...
            print("initial testing accuray,",gA_test)
            print("initial testing loss,",gL_test)
            ...
            for j, (X, lS_o, lS_i, T) in enumerate(train_ld):
                ...
                if not args.inference_only:
                    ...
                    for i in range(len(dlrm_weights_mask)):
                        cur_mask = dlrm_weights_mask[i]
                        axis_0_norm = torch.norm(cur_mask,dim=0)
                        axis_1_norm = torch.norm(cur_mask,dim=1)
                        flag1 = (axis_0_norm != 0)
                        flag2 = (axis_1_norm != 0)
                        if torch.sum(flag1).item() == 0:
                            dlrm_weights_mask[i][:,0] = 1
                        if torch.sum(flag2).item() == 0:
                            dlrm_weights_mask[i][0, :] = 1
                    ## corner case protection, in case the whole layer of the network is removed        
                    for i,w in enumerate(dlrm_weights):
                        w.data *= dlrm_weights_mask[i].cuda()
                ...
                should_test = (
                    ...
                    and (((j + 1) % args.test_freq == 0) or (j + 1 == nbatches)) # the last one is to test the accuracy directly after pruning
                )
