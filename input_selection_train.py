...
def get_bncp_layers_dict(net_model, arg=None):
    ...
    for name, m in net_model.named_modules():
        print('name',name)
        print("m",m)
        if name == "top_l.0":  #we only need to process the first layer
            layer_names[m] = name
...
if __name__ == "__main__":
    ...
    if args.exp_mode == "input_selection":
                ...
                if should_test and not args.inference_only:
                    ...
                    if should_print and should_test and is_rctrain:
                        if len(save_budgets) > 0 and cur_resource <= save_budgets[-1]:
                            ...
                            save_budgets.pop()
                            save_model_path = os.path.join(args.save_result_dir, 'optmodel_budget_{}_s1_{}.pt'.format(round(cur_resource, 3),round(s_data_temp[0],3)))
