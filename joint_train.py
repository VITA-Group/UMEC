...
def get_bncp_layers_dict(net_model, arg=None):
    ...
    """
    rctrain part 1: check bncp_layers
    """

    print('Part 1: Check bncp_layers')
    print('*'*40)
    layer_names = {}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mmpretrain', default=None,
                        help='file to load pretrained minimax model')
    parser.add_argument('--pretrain', default=None,
                        help='file to load pretrained model')
    parser.add_argument('--loadsub', action='store_true',
    ...
    parser.add_argument('--dense', action='store_true',
                        help='densely train the network')
    parser.add_argument('--ft', action='store_true',
                        help='fine-tune mode')
    parser.add_argument('--budget', default=0.5, type=float,
                        help='resource budget (relative)')
    parser.add_argument('--zclamp', action='store_true',
                        help='clamp z to keep decreasing s')
    parser.add_argument('--eval_tr', action='store_true',
                        help='evaluate training set')
    parser.add_argument('--sl2wd', default=0.0, type=float,
                        help='l2 weight decay for s')
    parser.add_argument('--early_stop', action='store_true',
                        help='early stop when resource budget is achieved')
    parser.add_argument('--verbose', default=True,action='store_true',
                        help='set verbose to be true to print rctrain infos')
    ############  ********** ********** ********** ********** ********** ********** ********** ********** ############
    ...
    if is_rctrain:
        # modify 1
        layer_names, bncp_layers, bncp_layers_dict = get_bncp_layers_dict(dlrm, args)

        # modify 2
        minimax_model, dual_optimizer, s_optimizer = build_minimax_model(dlrm, layer_names, bncp_layers,bncp_layers_dict, args)

        print("layer_names",layer_names)
        print("bncp_layers",bncp_layers)
        print("bncp_layers_dict",bncp_layers_dict)
        ...
        while k < args.nepochs:
            ...
            for j, (X, lS_o, lS_i, T) in enumerate(train_ld):
                ...
                if should_print or should_test:
                    ...
                    all_gA_train.append(gA)
                    all_gL_train.append(gL)

                    if is_rctrain:
                        all_cur_resource.append(cur_resource)
                        all_s1.append(s_data[0])
                        all_s2.append(s_data[1])
                        all_s3.append(s_data[2])


                    np.savetxt(args.save_result_dir+"all_gA_train.txt",all_gA_train,fmt="%.6f")
                    np.savetxt(args.save_result_dir+"all_gL_train.txt",all_gL_train,fmt="%.6f")

                    if is_rctrain:
                        np.savetxt(args.save_result_dir+"all_cur_resource.txt",all_cur_resource,fmt="%.6f")
                        np.savetxt(args.save_result_dir+"all_s1.txt",all_s1,fmt="%.3f")
                        np.savetxt(args.save_result_dir+"all_s2.txt",all_s2,fmt="%.3f")
                        np.savetxt(args.save_result_dir+"all_s3.txt",all_s3,fmt="%.3f")
                ...
                if should_test and not args.inference_only:
                    ...
                    all_gA_test.append(gA_test)
                    all_gL_test.append(gL_test)
                    np.savetxt(args.save_result_dir+"all_gA_test.txt",all_gA_test,fmt="%.6f")
                    np.savetxt(args.save_result_dir+"all_gL_test.txt",all_gL_test,fmt="%.6f")

                    if is_best:
                        best_gA_test = gA_test
                        save_model_name = os.path.join(args.save_result_dir,"best.pt")
                        print("Saving model to {}".format(save_model_name))
                        print("Current best testing acc, ",gA_test)
                        torch.save(
                            {
                                "epoch": k,
                    ...
                    if should_print and should_test and is_rctrain:
                        if len(save_budgets) > 0 and cur_resource <= save_budgets[-1]:
                            s_data_temp = minimax_model.s.data.numpy()
                            save_budgets.pop()
                            save_model_path = os.path.join(args.save_result_dir, 'optmodel_budget_{}_s1_{}_s2_{}_s3_{}.pt'.format(round(cur_resource, 3),round(s_data_temp[0],3), round(s_data_temp[1],3),round(s_data_temp[2],3)))
                            # torch.save(minimax_model.state_dict(), save_model_path)
                            print("Saving model by the budget")
                            print("current resource,",round(cur_resource, 3))
                            print("Current training accuracy, ",gA)
                            print("Current testing accuracy, ",gA_test)

                            minimax_model_copy = copy.deepcopy(minimax_model)
                            make_zero_w(minimax_model_copy, optimizer)
                            ...
                            torch.save(
                                {
                                    ...
                                    "iter": j + 1,
                                    # "state_dict": dlrm.state_dict(),
                                    "state_dict": minimax_model_copy.net.state_dict(),
                                    "train_acc": gA,
                                    "train_loss": gL,
                                    "test_acc": gA_test,
                                    "test_loss": gL_test,
